...
One qualification of that durability comes from the accelerator, and it cuts the
other way. The anchored margin is robust to how the accelerator's signal is
smoothed (\cref{sec:invexpectations}), but it is load-bearing on how strong the
accelerator is. At the anchored configuration the margin resolves as falling
only for $\beta \ge 0.5$; below that it is unresolved, either because the anchor
sits inside the turning-point interval or because that interval is not estimable
(\cref{tab:b17}), and the default $\beta = 0.5$ sits on the boundary. Switching
the accelerator off makes the point sharply: at $\lambda_u = 0$ the turning
point is $\rhostar = 0.358$ for every $\beta$, with the anchor $0.363$ inside its
interval, so the margin does not invert, it vanishes; muting the accelerator the
other way, at $\beta = 0.05$, gives the same outcome. The durable half of the
hierarchy holds across elasticity, fiscal setting, price convention and
expectation gain, but it rests on an accelerator reactive enough to place the
turn to the right of the anchor, and that reactivity is the parameter
(\cref{sec:sa}) with the weakest empirical claim in the model.
