...
not: the loop turns because, at a fixed numeraire, the nominal wage \emph{is} the
real wage, so a wage cut is a real wage cut and no demand instrument can reach the
variable that moves. A price convention can. Relaxing the numeraire to complete
...
of the space is allowed to move. Third, it is conditional on the price numeraire:
...
What survives all three is not the \emph{position} of the frontier but the
anchored margin of the first result. Across fiscal settings, across the
marginalised space, and across both price conventions, the anchored retention
rate stays to the \emph{left} of the turning point for $\sigma \ge 0.4$, so the
