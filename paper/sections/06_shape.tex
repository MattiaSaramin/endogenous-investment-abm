...
first result of this paper is that the object being signed is a curve,
...
$\rhostar$, the argument minimum, inside the swept support in 19
...
with the quantity that proves economically decisive: which side of it
the anchored retention rate falls on, that rate being derived from the
investment-rate anchor in \cref{sec:anchoredrho} and bounded above by $0.363$.

\begin{table}[htbp]
\centering
\small
\caption{Turning point of $Y(\rho)$ and the position of the anchored retention
rate relative to it, at $\eta = 0$. The anchored $\rho$ is at most $0.363$,
derived in \cref{sec:anchoredrho}; every turn below is above that bound.}
\label{tab:turning}
\begin{tabular}{lrrrl}
\toprule
$\sigma$ & OLS slope & Chord & $\rhostar$ & Anchored $\rho$ lies\\
\midrule
...
Two regularities run down that table. The turning point rises with the
elasticity of substitution, from $\rhostar = 0.42$ at $\sigma = 0.30$ to
$0.64$ at $\sigma = 1.50$: the more readily capital substitutes for labor, the
further retention must go before the supply channel overtakes the demand
channel it displaces. And at every elasticity at which the turn is
resolved, the whole range $0.30$ to $1.50$, the anchored retention rate
lies to the left of the turn, on the falling branch. The margin is
comfortable rather than marginal: the lowest turn in \cref{tab:turning} is
$0.420$ against an anchored $\rho$ of at most $0.363$.

\begin{remark}
Retention depresses output while $\rho$ remains below a
$\sigma$-dependent threshold $\rhostar$, and expands it beyond; $\rhostar$
rises with $\sigma$, from $0.42$ at $\sigma = 0.3$ to $0.64$ at
$\sigma = 1.5$; and the anchored retention rate lies below $\rhostar$ at every
elasticity tested.
\end{remark}

The economic content is a statement about the margin against the
range. At the point where the data put $\rho$, the marginal effect of
...
\toprule
$\rho$ & $Y$ & $K$ & Employment & Unemployment & Wage share & Utilization\\
\midrule
...
output falls, from $96.7$ to $86.8$ (\cref{fig:retention}). This is the
strongest possible reconnection to \citeauthor{Teglio2025}'s leakage
mechanism: investment itself becomes the leak.

\citet{Teglio2025} anticipates this reading in the base paper's conclusions,
which conjecture that reinvesting capitalists' accumulated wealth is unlikely to
cure the shortfall in low-income households' consumption demand ``at least in a
model that does not envisage income growth''. Ours is exactly that model, 
constant $A$ and $g = 0$, so the conjecture is testable here, and the
mechanism measured above is its quantitative counterpart on the one channel the
model represents. The two propositions are adjacent, not identical, and the
distinction is worth stating in the same breath: the conjecture concerns the
wealth constraint on low-income households' spending, whereas what
\cref{tab:baseline} measures is $\mathrm{d}Y/\mathrm{d}\rho$ at the anchored
$\rho$; the negative margin there is the closest available counterpart to the
conjecture on the accumulation channel, not a test of the household constraint
itself. The conditioning clause it attaches, income growth ``large enough'', is exactly the no-growth limitation declared in \cref{sec:limits}, left here
...
weighted toward the branch below it. Presented on its own, this table invites
precisely the global label that \cref{sec:turning} withholds.
...
a frontier: the elasticity $\sigstar$ at which the estimated
...
the question is posed in the literature. But it is a reduction of the
...
\paragraph{The empirical range sits below the frontier} With $\sigma \in
...
anchored $\rho$, but the same curve summarized over a wider interval.
\paragraph{Unemployment has a different frontier} $\sigstar_U \approx
0.31$--$0.45$ lies well below $\sigstar_Y$. In the band $\sigma \approx
0.3$--$0.7$, output and unemployment rise together, growth with
...
\begin{figure}[htbp]
\centering
\includegraphics[width=\textwidth]{ces_sign_frontier.png}
\caption{Sign of $\mathrm{d}Y/\mathrm{d}\rho$ over the $(\sigma,\rho)$ grid. In
the map (left) blue is the wage-led region ($\mathrm{d}Y/\mathrm{d}\rho < 0$,
retention lowers output) and red the profit-led region; the black curve is the
zero contour---the sign frontier itself. The $\sigma$ grid is non-uniform (nodes
at $0.05, 0.20, 0.30, \dots, 1.25, 1.50$; $\rho$ at $0.30$--$0.60$), so ticks
fall only on simulated nodes and a dot marks each of the $31$ measured cells; the
remaining $9$ of $40$ are non-viable, carry no derivative, and are left blank. The
zero contour is interpolated between nodes, and its position between two $\sigma$
rows is read as such. The dashed line marks $\sigma = 1$. Steady-state output
against retention, one curve per $\sigma$ (right).}
\label{fig:frontier}
\end{figure}
...
conditioning dimensions move it materially. The choice of support: at
$c_0 = 1.0$, $\sigstar$ ranges from $0.46$ on $\rho \in [0.35, 0.55]$ to
$0.94$ on $\rho \in [0.45, 0.65]$. And the choice of normalization
