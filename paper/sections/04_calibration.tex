...
therefore assign every parameter to one of three anchoring tiers, and
state the tier wherever the parameter is used.

\paragraph{(1) Anchored} There exists an empirical quantity of which the
...
the replacement rate are in this tier. \paragraph{(2) Declared convention} A
value is needed, the literature offers a range rather than a point, and the
choice within that range is ours. These are not estimates and are never
described as such. $\delta$ is the consequential case. \paragraph{(3) Non-anchorable by decision} No empirical quantity corresponds to the
parameter at all, so searching for a citation would be cosmetic. $c_0$ is the
case, and it is handled by reporting both regimes throughout rather than by
choosing.
...
are modeling decisions that must not be presented as estimates.
...
$\rho$ (retention ratio) & $0.40$ & anchored through $I/Y$, not through
...
\addlinespace[4pt]
\multicolumn{3}{l}{Declared conventions, not estimates}\\
\addlinespace[2pt]
$\delta$ (depreciation) & $0.05$ & BEA-implied figure for this perimeter is
  $\approx 0.090$; structures alone $2$--$3\%$. A defensible midpoint,
  not a measurement. See \cref{sec:delta}.\\
$U_{\min}$, $w_{\min}$ & $0.01$, $0.45$ & guards; $w_{\min}$ never stably
  binds anywhere tested\\
$\max\tau$ (tax cap) & $0.6$ & guardrail; realized $\tau$ is a measured
  outcome\\
$\underline{I}$ (investment floor) & $0.1$ & numerical guardrail\\
$\bar{u}$ (target utilization) & $0.90$ & above the observed US average
  ($\approx 0.80$)\\
\addlinespace[4pt]
\multicolumn{3}{l}{Modelling choices, measured once, then frozen}\\
\addlinespace[2pt]
$(K_0,L_0)$ & $41.87$, $7.395$ & CES normalization anchor
  \citep{KlumpDeLaGrandville2000}; caveat per \citet{Temple2012}\\
$U_{\mathrm{REF}}$ & $0.26047$ & wage-curve normalization point.
  Not an estimate of the NAIRU.\\
$\bar{w}$ & $0.9$ & demoted to a normalization point once $\eta$ exists\\
$K_{\text{init}}$ & $40.0$ & selects the basin (multiple equilibria);
  held fixed in every sweep\\
$\beta$ (accelerator) & $0.5$ & no canonical value exists; treated by
  sensitivity analysis\\
\addlinespace[4pt]
\multicolumn{3}{l}{Declared non-anchorable}\\
\addlinespace[2pt]
$c_0$ (autonomous consumption) & $\{1.0, 2.0\}$ & model units have no exchange
  rate with data (numéraire $=1$). Its original justification was
  falsified; both regimes are reported everywhere.\\
\bottomrule
\end{tabularx}
\begin{tablenotes}[flushleft]\footnotesize
\item Note. One model period $=$ one year. This is a declared
...
each run are a convergence device, not 2{,}000 years of simulated
...
The capital block is anchored flows first: $\rho$ is anchored through
...
both sides of the ratio, private non-residential fixed capital, 
...
\citep{FRED_PNFI}. The model measured at $\rho = 0.40$ gives $0.158$ in
...
a labor market and no longer holds once profit is a residual
(\cref{app:retractions}).

\subsection{The anchored retention rate}
\label{sec:anchoredrho}

Several statements in this paper turn on where the empirically anchored
...
in support\\
Headline ($c_0 = 1.0$, $\sigma = 0.5$) & 0.161 & $< 0.35$ & below the
swept support\\
\bottomrule
\end{tabular}
\end{table}
...
paper's favor rather than against it. In the anchor scenario the band is
reached inside the support, at $\rho \approx 0.36$. In the headline scenario
measured $I/Y$ is already $0.161$ at $\rho = 0.35$, the lowest value
...
argument uses: the anchored $\rho$ is at most $0.363$. Wherever this
...
not derived at all. The reported figures below use the derived bound.
...
``toward the center of the literature'' late in a project: doing so would move
...
$g = 0$, so $K/Y$ is not an anchor at all but a mechanical outcome:
$K/Y = (I/Y)/\delta$, and measured $I/K$ is $0.0500$, exactly $\delta$. Model
$K/Y$ is therefore $3.17$--$3.64$ against a business-sector $1.23$
\citep{FRED_NetStock}, and no choice of $\rho$ reconciles the two. The model cannot match business
...
$c_0$ is declared non-anchorable by decision: it is autonomous
