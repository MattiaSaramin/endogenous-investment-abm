...
\begin{declbox}[frametitle={How ``wage-led'' and ``profit-led'' are used in this
paper}]
The first bullet is not a caveat on the frontier; it is a statement about what
kind of object the response is. Because $Y(\rho)$ turns inside the support, no
single slope characterises it, and the sign of any slope depends on the
interval chosen --- as \cref{sec:bridge} later demonstrates by moving
$\sigstar$ from $0.643$ to $0.447$ using nothing but a different interval on
the same data.

We therefore treat the \emph{primary} statement about this model as a statement
about the turn: where the turning point $\rhostar$ lies, and on which side of
it the empirically anchored range of $\rho$ falls (\cref{tab:turning}). The
labels ``wage-led'' and ``profit-led'' are used throughout as \textbf{local
descriptions of one side of that turn under a declared estimator}, never as a
global verdict on the economy. Where a label appears without that
qualification it should be read as shorthand for the whole-support OLS slope
at the stated $(\sigma, c_0, \eta)$, which is the estimator that defines
$\sigstar$ everywhere in this paper.
\end{declbox}
