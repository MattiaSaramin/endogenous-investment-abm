...
![Y(rho) turns inside the support, and rho* rises with sigma](results/ces_nb_u_curve.png)

*(Figure produced by `notebooks/01_Endogenous_Investment.ipynb` from the committed
20-seed panel and `ces_b14_taskB_slopes.csv` - the only figure in this README generated by
the notebook rather than by a driver script.)*
...
# reproduce the figures and analysis. Two small live sweeps (3 seeds, ~2 min); every
# brief-07..14 section reads the committed CSVs rather than re-simulating ~8 h of runs.
